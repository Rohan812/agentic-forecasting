...
- **AI stocks / NVDA (in progress, branch `ai-stocks-poc`).** [`implementations/ai_stocks_forecasting/`](../implementations/ai_stocks_forecasting/) transposes the energy/oil stack — the repo's other daily, news-driven, shock-prone series — onto NVDA. Its distinguishing idea is a **discovery loop**: a study-mode agent proposes candidate news patterns, a non-LLM statistics module (`signals.py`) scores each one (precision vs. base rate, lift, held-out validation, Fisher's exact test), and only patterns that clear the gate are written to a master strategy file that the forecaster reads at forecast time. Per-experiment files keep the full research trail — candidates considered and rejections with reasons — so a graduated pattern is auditable and later study sessions do not re-cover the same ground. Currently a scaffold: the energy modules are copied with imports rewired, and the NVDA data path, specs, statistics module, and retargeted prompts are still to come.
