...
`ai_stocks_forecasting/` is a **work-in-progress scaffold**, not yet a finished reference implementation: it is a copy of the energy/oil Python modules with the package name rewired. Its data path, specs, shock definition, numerical baselines, statistics contract (`signals.py`), analyst prompt strings, news-grounded configs (`build_nvda_news_config`, `build_nvda_multitask_news_config`), prompt builders, shock task, predictor and outcome series (with a ten-origin scored smoke backtest), and gate-enforced master-strategy schema (`NvdaStrategyState`) are retargeted (`build_nvda_service()` registers NVDA's adjusted close as `nvda_stock_price`). The other factory names, the scenario task, the adaptive agent, and the skills are still WTI-targeted. See [`ai_stocks_forecasting/README.md`](ai_stocks_forecasting/README.md) for what remains.
